...
describe('MessageServer broadcast', () => {
    ...
    it ('should call send method of each WebSocket in clients', function () {
        ...
        const message1 = "my first message"
        const message2 = "my second message"
        const msg1 = JSON.stringify([message1])
        const msg2 = JSON.stringify([message2, message1])
        ms.addMessage(message1)
        ms.broadcast()
        ms.addMessage(message2)
        ms.broadcast()
        expect(this.player1.send.calls.allArgs()[0]).toEqual([msg1])
        expect(this.player2.send.calls.allArgs()[0]).toEqual([msg1])
        expect(this.player3.send.calls.allArgs()[0]).toEqual([msg1])
        expect(this.player1.send.calls.allArgs()[1]).toEqual([msg2])
        expect(this.player2.send.calls.allArgs()[1]).toEqual([msg2])
        expect(this.player3.send.calls.allArgs()[1]).toEqual([msg2])
    })
})